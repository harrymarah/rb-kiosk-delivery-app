      ...
      <CardContent className="p-2 flex flex-col flex-1">
        <div className="relative mb-2">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="absolute top-1 right-1 h-6 w-6 bg-gray-100/90 hover:bg-gray-200/90 backdrop-blur-sm"
            onClick={(e) => {
              e.stopPropagation();
              onToggleFavorite?.();
            }}
        ...
        <div className="space-y-1 flex-1 flex flex-col justify-end">
          <h3 className="font-medium text-xs leading-tight text-foreground line-clamp-2">{name}</h3>
          ...
          {isNewArrival && (
            <div className="flex items-center gap-1">
              <Heart className="h-3 w-3 shrink-0 text-destructive fill-destructive" />
              <span className="text-[10px] font-medium leading-none text-destructive">New</span>
            </div>
          )}
          ...
          {offer && !isNewArrival && (
            <div className="flex items-center gap-1">
              <Heart className="h-3 w-3 shrink-0 text-destructive fill-destructive" />
              <span className="text-[10px] font-medium leading-none text-destructive line-clamp-1">{offer}</span>
            </div>
          )}
          ...
          {onAddToCart && (
            <Button
              size="sm"
              className="w-full mt-1 h-7 px-1 text-[11px]"
              onClick={(e) => {
                e.stopPropagation();
                onAddToCart();
              }}
